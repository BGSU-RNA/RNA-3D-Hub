...
;(function($) {
    ...
    var jmolModel = {
        ...
        downloadSuperposition: function() {
            t1 = $.jmolTools.numModels
            console.log(t1)
    ...
    var Helpers = {
        ...
        updateDownloadButtonState: function() {
            var checkedCount = $('.jmolInline:checked').length;
            if (checkedCount < 2) {
                $('#downloadSuperposition').prop('disabled', true);  // Disable if fewer than 2 are checked
            } else {
                $('#downloadSuperposition').prop('disabled', false);  // Enable if 2 or more are checked
            }
        },
        ...
        bindEvents: function() {
            ...
            */
            ...
            // Add binding for Download Superposition button
            $('#downloadSuperposition').on('click', function() {
                var selectedModels = [];

                // Collect checked models
                $('.jmolInline:checked').each(function() {
                    var modelId = $(this).attr('id');  // Get model ID from checkbox ID
                    var model = $.jmolTools.models[modelId];  // Retrieve corresponding model
                ...
                // Call the download function with the selected models
                if (selectedModels.length > 1) {
                    selectedModels[0].downloadSuperposition(selectedModels);
                }
            });

            // Update download button when checkboxes change
            $('.jmolInline').on('change', function() {
                Helpers.updateDownloadButtonState();
            });
