...
class Motif_model extends CI_Model {
    ...
    function get_history()
    {
        ...
        for ($i = 0; $i < count($result); $i++) {
            ...
            $link = "http://rna.bgsu.edu/MotifAtlas/index.php/motif/view/{$prev_release}/{$result[$i]['motif_id2']}";
...
//                                str_replace(',',', ',$result[$i]['two_minus_one'])
    ...
    {        
        $this->db->select('t1.position')->from('loop_positions as t1');
        $this->db->join('loop_positions as t2','t1.loop_id=t2.loop_id AND t1.nt_id=t2.nt_id');
...
// ORDER BY field (position,3,4,5,1,2);
        ...
        $this->db->select()->from('loop_positions')
...
//         ksort($nts);
    ...
    {   
        ...
        foreach ($loops as $loop) {
            $this->db->select()->from('mutual_discrepancy');
...
//                 $checkboxes .= 'data-nt="'. implode(',',$corr_positions) . '">';
    ...
    {
        $this->db->select('*');
        $this->db->from('mutual_discrepancy');        
        $this->db->where('release_id', $this->release_id);
