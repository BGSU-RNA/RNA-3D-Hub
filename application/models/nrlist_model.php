...
function add_url($n)
{
    return anchor(site_url(array('nrlist','view',$n)), $n);
}
...
class Nrlist_model extends CI_Model {
    ...
    function get_releases_by_class($id)
    {
        ...
        foreach ($query->result() as $row) {
            ...
            $releases[0][] = anchor("nrlist/release/".$row->release_id, $row->release_id);
    ...
    function get_history($id,$mode)
    {
        ...
        foreach ($query->result() as $row) {
            $table[] = array($row->nr_class1,
                             anchor("nrlist/view/".$row->nr_class2,$row->nr_class2),
                             anchor("nrlist/release/".$row->release_id, $row->release_id),
    ...
    function get_all_releases()
    {
        ...
        foreach ($query->result() as $row) {
            if ($i == 0) {
                $id = anchor("nrlist/release/".$row->id, $row->id.' (current)');
                $i++;
            } else {
                $id = anchor("nrlist/release/".$row->id, $row->id);
            }
            if (array_key_exists($row->id,$changes)) {
                $label = $this->get_label_type($changes[$row->id]);
                $compare_url = site_url(array('nrlist','compare',$row->id,$releases[$row->id]));
                $status = "<a href='$compare_url'><span class='label {$label}'>{$changes[$row->id]} changes</span></a>";
            } else {
                $status = '';
            }
        ...
        foreach ($query->result() as $row) {
            if ($row->nr_release_id2 == $releases[$row->id]) {
                $tables[$row->resolution][] = array(
                    anchor(site_url(array('nrlist','compare',$row->id,$releases[$row->id])), $row->id),
    ...
    function add_label($class_id,$changed)
    {
        if (array_key_exists($class_id,$changed)) {
            return " <span class='label warning'>Changed</span>";
        } else {
            return '';
        }
    }
    ...
    function get_release($id,$resolution)
    {
        ...
        // get parents
        $changed = array();
        $this->db->select('nr_parents.class_id as class_id,nr_parents.parents as parents')
                 ->from('nr_classes')
        ...
        foreach ($order as $class_id) {
            ...
            $table[] = array($i,
                             anchor("nrlist/view/".$class_id,$class_id),
//                              $this->make_pdb_widget_link($pdb_id),
                             $this->add_label($class_id,$changed),
                             $pdb_id,
                             $pdb[$pdb_id]['title'],
