...
class motifs_model extends Model {

    public array $types = [];

    function __construct()
    {
        $this->types = array('HL','IL','J3');
        parent::__construct();
    }
    ...
    function get_current_release_info()
    {
        ...
        $data['release_info']['j4_release'] = $this->get_latest_release('j4');
        $data['release_info']['j5_release'] = $this->get_latest_release('j5');
        $data['release_info']['j6_release'] = $this->get_latest_release('j6');
        $data['release_info']['j7_release'] = $this->get_latest_release('j7');
        $data['release_info']['j8_release'] = $this->get_latest_release('j8');
        $data['release_info']['j9_release'] = $this->get_latest_release('j9');
    ...
    function get_featured_motifs($motif_type)
    {
        ...
        $builder = $this->db->table('ml_motif_annotations AS MA');

        foreach($motifs as $motif) {
            $query=$builder->select('MA.motif_id')
            ...
            if ($query_result != null) {
                $data[$motif] = $query_result->row()->motif_id;
            }
        }
    ...
    function get_all_motifs($release_id, $motif_type) {
        // gets all motif_id values for this release and motif type
    ...
    function get_annotations($release_id, $motif_type)
    {
        // get all motif groups, loops in each group, annotation of each loop
        // not every loop has an annotation, so use left join
        $query = $this->db->table('ml_loops')
                 ->select('ml_loops.motif_id, ml_loops.loop_id, ml_loops.ml_release_id, loop_annotations.annotation_1')
                 ->join('loop_annotations', 'ml_loops.loop_id = loop_annotations.loop_id', 'left')
                 ->where('ml_release_id', $release_id)
                 ->where('ml_loops.loop_id LIKE', $motif_type . '%', 'after')
                 ->orderby('motif_id, annotation_1')
                 ->get()
                 ->getResult();

        // return an array of strings of tab-separated values
        // leave an extra tab at the end of each line for adding mapped loop annotations
        $loop_data = array();

        foreach($query as $row) {
            $loop_data[] = $row->motif_id . "\t" . $row->loop_id . "\t" . $row->annotation_1 . "\t";
        }
    ...
    function db_get_all_releases($motif_type)
    {
        $query = $this->db->table('ml_releases AS MR')
                 ->select('STRAIGHT_JOIN MR.ml_release_id, MR.type, MR.date, MR.description, MR.annotation, MR.index, MR.nr_release_id, MR.parent_ml_release_id, count(ML.loop_id) AS loops, count(DISTINCT(motif_id)) AS motifs', FALSE)
                 ->join('ml_loops AS ML','MR.ml_release_id = ML.ml_release_id')
                 ->where('MR.type',$motif_type)
                 ->like('ML.loop_id',$motif_type,'after')
                 ->groupby('MR.ml_release_id')
                 ->orderby('MR.index','desc')
                 ->get();
        return $query;
    }
    ...
    function get_polymorphs($motif_type, $release_id)
    {
        // Build the query
        $query_string = "
            SELECT seq, length, GROUP_CONCAT(motif_id) AS motifs, COUNT(motif_id) AS motif_num
            FROM (
                SELECT DISTINCT(seq AND motif_id), seq, length, motif_id
                FROM ml_loops AS t1
                JOIN loop_info AS t2 ON t1.loop_id = t2.loop_id
                WHERE t1.ml_release_id = ?
                AND t2.`type` = ?
        ...
        $query = $this->db->query($query_string, [$release_id, $motif_type]);
        ...
        //     ORDER BY length DESC;
        // ";
        // $query = $this->db->query($query_string, FALSE)->get();

        if (count($results) == 0) { return 'No polymorphs found in this release'; }
    ...
    {
        $query = $this->db->table('ml_release_diff')
                 ->select('release_id1')
                 ->selectSum('num_added_groups','nag')
                 ->selectSum('num_removed_groups','nrg')
                 ->selectSum('num_updated_groups','nug')
                 ->where('type',$motif_type)
    ...
    function get_release_status($motif_type,$id)
    {
        ...
        $current_release = $query->ml_release_id;
    ...
    {
         $image = config('BGSUConfig')->hub_url . '/img/MotifAtlas/' . strtoupper($motif_type) . $release_id . '/' . $id . '.png';
    ...
    function get_loop_annotations($motif_id, $id)
    {
        ...
        $builder = $this->db->table('loop_annotations as la');
        $query=$builder->select('la.loop_id, la.annotation_1, la.annotation_2')
    ...
    function get_num_core_nts($motif_id) {
        // for the given $motif_id, find the largest position value in ml_loop_positions
        $query = $this->db->table('ml_loop_positions AS ML')
                 ->selectmax('ML.position')
                 ->where('motif_id', $motif_id)
                 ->get()
                 ->getResult();

        // return a number or null, which displays and sorts well
        return $query[0]->position;

        // if (empty($query) || $query[0]->position === null) {
    ...
    // This code creates the table that constitutes an HL, IL, J3, ... release
    function get_release($motif_type,$id) {
        // $id is the ml_release_id

        // get list of motif groups, updated/>2 parents etc
        $query = $this->db->table('ml_motifs_info')
                    ->select('motif_id, comment')
                    ->where('type',$motif_type)
                    ->where('ml_release_id',$id);
        $query_result = $query->get()->getResult();
        ...
        // count all loop comment types for this motif
        // displays under coordinate window but that is probably not the best place
        ...
        $query = $this->db->table('ml_loops')
                        ->select('motif_id, count(loop_id) AS instances')
        ...
        foreach ($query_result as $row) {
            ...
            */
            ...
            // the following line is returning 0 for some motif groups that lost data
            // it's overkill for what we need here
            // $length_distribution = $this->_get_motif_length_distribution($row->motif_id, $id);

            // write new code to simply return the number of core positions in the group
            // that is the maximum position value in ml_loop_positions
            $num_core_positions = $this->get_num_core_nts($row->motif_id);
    ...
    function _get_motif_length_distribution($motif_id, $release_id) {
        ...
        $query=$builder->select('LI.length')
        ...
        $query = $builder->select('COUNT(MLP.unit_id) AS unit_count')
                        ->where('MLP.ml_release_id', $release_id)
                        ->where('MLP.motif_id', $motif_id);
        $query_result = $query->get()->getResult();

        if (!empty($query_result)) {
            $distribution['unit_count'] = $query_result[0]->unit_count;
        } else {
            $distribution['unit_count'] = 0;
        }
    ...
    function get_compare_radio_table() {
        ...
        foreach ($this->types as $motif_type) {
            ...
            foreach ($query->getResult() as $row) {
                ...
                $attributes = array('name'=>'release1','value'=>$row->ml_release_id);
                $table[$motif_type][] = form_radio($attributes) . $row->ml_release_id . $num_changes;

                $attributes = array('name'=>'release2','value'=>$row->ml_release_id);
                $table[$motif_type][] = form_radio($attributes) . $row->ml_release_id;
            }
        }
    ...
    function get_latest_release($motif_type, $date=NULL)
    {
        ...
        $query = $builder->select('ml_release_id, date')
                        ->where('type',$motif_type)
                        ->orderby('index','desc')
                        ->limit(1)
                        ->get();
    ...
    function get_new_and_replaced_pdbs($pdbs1, $pdbs2)
    {
        ...
        foreach($only_new as $new_id) {
            foreach($only_old as $old_id) {
                ...
                $query = $this->db->table('nr_pdbs AS P1')
                         ->select('P1.pdb_id')
