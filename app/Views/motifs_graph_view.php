...
<script>
    ...
    $(document).ready(function() {
        ...
        function show_motif_exemplar_in_jmol(id) {
            $.post('<?=$baseurl?>' + get_rna3dhub_environment() + '/ajax/get_exemplar_coordinates', { motif_id: id }, function(data) {
                jmolScript('zap;');
        ...
        vis.ready(function() {
            ...
            function handle_edge_click(event) {
                 ...
                 var text = 'Motifs <a href="<?=$baseurl?>' + env + '/motif/view/' + target.data.source + '" target="_blank">' + target.data.source + '</a>';
                 text += ' and ' + '<a href="<?=$baseurl?>' + env + '/motif/view/' + target.data.target + '" target="_blank">' + target.data.target + '</a>';
                 text += ' are connected by loops ';
                 text += target.data.connection.split(' ').join(' and ');
                 text += ' at discrepancy ' + target.data.disc.toFixed(4) + '.';
                 text +=  ' <a href="<?=$baseurl?>' + env + '/motif/compare/'+ target.data.source + '/' + target.data.target + '" target="_blank">Compare motifs</a>';
                 $('#signature').html(text);

                 text =  '<img class="thumbnail" src="<?=$baseurl?>img/MotifAtlas/<?=$img_loc?>/'+target.data.source+'.png"/>';
                 text += '<img class="thumbnail" src="<?=$baseurl?>img/MotifAtlas/<?=$img_loc?>/'+target.data.target+'.png"/>'
            ...
            function handle_node_click(event) {
                 var target = event.target;
                 var env = get_rna3dhub_environment();
                 var text = 'Motif <a href="<?=$baseurl?>' + env + '/motif/view/' + target.data.id + '" target="_blank">' + target.data.id + '</a>';
                 ...
                 $('#varna').html('<img class="thumbnail" src="<?=$baseurl?>img/MotifAtlas/<?=$img_loc?>/'+target.data.id+'.png"/>').show();
            }
